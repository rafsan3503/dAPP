...
import maticLogo from "../assets/PolygonLogo.png";
import avaxLogo from "../assets/Avalanchelogo.png";
import ftmLogo from "../assets/Fantomlogo.png";
import busdLogo from "../assets/BUSDlogo.png";
import linkLogo from "../assets/linkLogo.png";
import adaLogo from "../assets/Adalogo.png";
import atomLogo from "../assets/Cosmoslogo.png";
import solLogo from "../assets/Solanalogo.png";
import sandLogo from "../assets/SandboxLogo.png";
import mboxLogo from "../assets/MoboxIcon.png";
...
const bearHeldByInvestors = async () => {
  ...
  const marketingBear =
    (await bear.balanceOf(marketingBearAddress)) / 1000000000;
  return (
    bearTotalSupply -
    bearLPToken -
    unicryptBear -
...
const treasuryMatic = async () => {
  const maticBalance = ethers.utils.formatEther(
    await matic.balanceOf(treasuryAddress)
  );
...
const treasuryFtm = async () => {
  const ftmBalance = ethers.utils.formatEther(
    await ftm.balanceOf(treasuryAddress)
  );
...
const treasuryLink = async () => {
  const linkBalance = ethers.utils.formatEther(
    await link.balanceOf(treasuryAddress)
  );
...
const treasuryAda = async () => {
  const adaBalance = ethers.utils.formatEther(
    await ada.balanceOf(treasuryAddress)
  );
...
const treasuryAtom = async () => {
  const atomBalance = ethers.utils.formatEther(
    await atom.balanceOf(treasuryAddress)
  );
...
const treasurySol = async () => {
  const solBalance = ethers.utils.formatEther(
    await sol.balanceOf(treasuryAddress)
  );
...
const treasurySand = async () => {
  const sandBalance = ethers.utils.formatEther(
    await sand.balanceOf(treasuryAddress)
  );
...
const treasuryMbox = async () => {
  const mboxBalance = ethers.utils.formatEther(
    await mbox.balanceOf(treasuryAddress)
  );
...
const treasuryIbat = async () => {
  const ibatBalance =
    ethers.utils.formatEther(await ibat.balanceOf(treasuryAddress)) *
    1000000000;
      ...
      <div className="flex mf:flex-row flex-col items-start justify-between pt-10 px-4">
        <div className="flex flex-1 justify-center items-start flex-col mf:mr-10">
          ...
          <div className="grid grid-cols-4 items-center lg:grid-cols-5 w-full mt-10 border-2 p-5 rounded-xl gap-4 text-white text-sm md:text-xl backdrop-blur-xl text-center divide-x">
            <div>
              <img
                src={maticLogo}
                alt="logo"
                className="w-12 lg:w-16 border-2"
              />
            </div>
            <div>Polygon</div>
            ...
            <div>
              <img src={ftmLogo} alt="logo" className="w-12 lg:w-16 border-2" />
            </div>
            <div>Fantom</div>
            ...
            <div>
              <img src={adaLogo} alt="logo" className="w-12 lg:w-16 border-2" />
            </div>
            <div>Cardano</div>
          ...
          <div className="grid grid-cols-4 lg:grid-cols-5 items-center border-2 rounded-lg w-full mt-10 text-white text-sm md:text-xl backdrop-blur-xl p-5 text-center divide-x gap-4">
            ...
            <div>
              <img src={logo} alt="logo" className="w-12 lg:w-16 border-2" />
            </div>
